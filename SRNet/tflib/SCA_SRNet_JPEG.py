...
class SCA_SRNet(Model):
    def _build_model(self, input_batch):
        ...
        with arg_scope([layers.conv2d], num_outputs=16,
            ...
            arg_scope([layers.batch_norm], decay=0.9, center=True, scale=True,
                      updates_collections=None, is_training=self.is_training, fused=True,
                      data_format=self.data_format), \
            arg_scope([layers.avg_pool2d],
                      kernel_size=[3, 3], stride=[2, 2], padding='SAME', data_format=self.data_format):
            with tf.variable_scope('Layer1'):  # 256*256
                W = tf.get_variable('W', shape=[3, 3, 1, 64],
                                    initializer=layers.variance_scaling_initializer(),
                                    dtype=tf.float32, regularizer=layers.l2_regularizer(5e-4))
                b = tf.get_variable('b', shape=[64], dtype=tf.float32,
                                    initializer=tf.constant_initializer(0.2))
                conv = tf.nn.bias_add(tf.nn.conv2d(tf.cast(_inputs_image, tf.float32), W, [1, 1, 1, 1], 'SAME',
                                                   data_format=self.data_format), b, data_format=self.data_format,
                                      name='Layer1')
                actv = tf.nn.relu(conv)
                prob_map = tf.sqrt(tf.nn.conv2d(tf.cast(_inputs_Beta, tf.float32), tf.abs(W), [1, 1, 1, 1], 'SAME',
                                                data_format=self.data_format))
